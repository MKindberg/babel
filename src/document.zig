...
pub const Document = struct {
    ...
    /// Get a list of TextEdits required to transform the text of the current document into newText.
    pub fn transform(self: Document, allocator: std.mem.Allocator, newText: []const u8) !std.ArrayList(types.TextEdit) {
        return try transformText(allocator, self.text, newText);
    }

    /// Get a list of TextEdits required to transform 'old` to `new`.
    pub fn transformText(allocator: std.mem.Allocator, old: []const u8, new: []const u8) !std.ArrayList(types.TextEdit) {
        var edits = try diff(allocator, old, new);
        defer edits.deinit(allocator);
        var res = std.ArrayList(types.TextEdit).empty;
        for (edits.items) |e| {
            switch (e) {
                .Insert => |i| {
                    const pos = Document.idxToPosText(old, i.idx).?;
                    try res.append(allocator, .{ .newText = new[i.start..i.end], .range = .{ .start = pos, .end = pos } });
                },
                .Delete => |d| {
                    const start = Document.idxToPosText(old, d.start).?;
                    const end = Document.idxToPosText(old, d.end).?;
                    try res.append(allocator, .{ .newText = "", .range = .{ .start = start, .end = end } });
                },
                .Equal => {},
            }
        }
