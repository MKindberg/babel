...
pub const Document = struct {
    ...
    pub fn diff(self: Document, allocator: std.mem.Allocator, new: []const u8) !std.ArrayList(types.TextEdit) {
        return try diffText(allocator, self.text, new);
    }

    pub fn diffText(allocator: std.mem.Allocator, old: []const u8, new: []const u8) !std.ArrayList(types.TextEdit) {
        var arr = try shortestEdit(allocator, old, new);
        defer {
            for (arr.items) |a| allocator.free(a);
            arr.deinit(allocator);
        }
        var edits = try backtrack(allocator, old, new, arr);
